...
実装後の `review-code` は4ラウンド回した。ここで新たに見つかった欠陥クラスを記録しておく。**「設計を無効化する Critical はループで直さない」という出口を、finding を反映するステップの後ろに置くと機能しない** — 出口に到達する前に設計が書き換えられてしまう。加えて finding の却下理由（誤り / レビュアーの好み / 依頼の範囲外）は閉じた列挙なので、正当な設計無効化 Critical はどれにも当たらず必ず「受理」になり、無条件の反映ステップに掃き込まれる。よって**反映ステップそのものに例外を書く必要がある**。この欠陥は `review-code` の Pass、`plan-work` の Pass、`implement-work` の Plan gate の3箇所に同じ形で存在していた。出口を列挙するだけでは足りず、その出口へ分岐しうる全ステップから参照させること。
