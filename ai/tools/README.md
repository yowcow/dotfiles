...
**ただし走査は非再帰であり、subagent の消費は計上されない。** Claude Code は subagent のターンを親の transcript にインラインで書かず、`<project-dir>/<session-uuid>/subagents/agent-*.jsonl` に書く。`*.jsonl` の glob は非再帰なのでこれらは 1 件も読まれず、親側の assistant レコードは `isSidechain` がすべて false であるため、消費がインラインで現れることもない。固定窓 `--until 2026-09-02T00:00:00Z` の既定ディレクトリで実測すると、top-level の `claude-sonnet-5` が 0 件であるのに対し `subagents/` には 5,515 件あり（`requestId` の重なりは 588 ファイル中 1 件）、未計上額は真の合計の 14〜17% にあたる。**`dispatch total` は表示されるのに、その dispatch が消費した token は 1 つも計上されていない** — 出力だけを見て気づける形になっていない。[yowcow/dude#159](https://github.com/yowcow/dude/issues/159) の測定報告が subagent 消費量を 8 標本からの外挿として別扱いしたのは、この穴の裏返しである。**本スクリプトの導入以前からある穴であり、この差分は塞いでいない** — 塞ぐには `sessions` の分母と [yowcow/dude#161](https://github.com/yowcow/dude/issues/161) の判定指標の母数をどう扱うかの判断が要る。
