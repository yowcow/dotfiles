#!/usr/bin/env bash
# Table test for ai/skills/pr-to-ready/scripts/resolve-pr-base.sh: how far the
...
#
# RED verification (see ai/tests/README.md) -- the trailer scan used to walk to
...
#   tmp="$(mktemp -d)"
#   git show bb8d8b8^:ai/skills/pr-to-ready/scripts/resolve-pr-base.sh >"$tmp/old.sh"
#   SUT="$tmp/old.sh" ai/tests/run.sh ai/tests/pr-to-ready/resolve-pr-base_test.sh
set -euo pipefail
...
SUT="${SUT:-${REPO_ROOT}/ai/skills/pr-to-ready/scripts/resolve-pr-base.sh}"
