#!/usr/bin/env bash
# Table test for ai/skills/pr-to-ready/scripts/watch-copilot-review.sh: each row
...
# script) — see ai/tests/README.md. The sibling has to be copied next to the
...
#   tmp="$(mktemp -d)"
#   git show 9600d30^:ai/skills/pr-to-ready/scripts/watch-copilot-review.sh >"$tmp/watch-copilot-review.sh"
#   cp ai/skills/pr-to-ready/scripts/list-copilot-reviews.sh "$tmp/"
#   SUT="$tmp/watch-copilot-review.sh" ai/tests/run.sh ai/tests/pr-to-ready/watch-copilot-review_test.sh
set -euo pipefail
...
SUT="${SUT:-${REPO_ROOT}/ai/skills/pr-to-ready/scripts/watch-copilot-review.sh}"
