#!/usr/bin/env bash
# Table test for ai/skills/pr-to-ready/scripts/request-copilot-review.sh: each
...
#   tmp="$(mktemp -d)"
#   git show 1245a06^:ai/skills/pr-to-ready/scripts/request-copilot-review.sh >"$tmp/old.sh"
#   SUT="$tmp/old.sh" ai/tests/run.sh ai/tests/pr-to-ready/request-copilot-review_test.sh
set -euo pipefail
...
SUT="${SUT:-${REPO_ROOT}/ai/skills/pr-to-ready/scripts/request-copilot-review.sh}"
