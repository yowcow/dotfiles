#!/usr/bin/env bash
# Table test for ai/skills/pr-to-ready/scripts/retarget-pr.sh: the two gates it
...
#
# RED verification (see ai/tests/README.md) -- against the script as it was
...
#   tmp="$(mktemp -d)"
#   git show 80376f3^:ai/skills/pr-to-ready/scripts/retarget-pr.sh >"$tmp/old.sh"
#   SUT="$tmp/old.sh" ai/tests/run.sh ai/tests/pr-to-ready/retarget-pr_test.sh
set -euo pipefail
...
SUT="${SUT:-${REPO_ROOT}/ai/skills/pr-to-ready/scripts/retarget-pr.sh}"
...
# loudly here instead of quietly operating on the dotfiles checkout.
