#!/usr/bin/env bash
# Table test for ai/skills/pr-to-ready/scripts/resolve-thread.sh: each row is a
...
#   tmp="$(mktemp -d)"
#   git show 8cd2953^:ai/skills/pr-to-ready/scripts/resolve-thread.sh >"$tmp/old.sh"
#   SUT="$tmp/old.sh" ai/tests/run.sh ai/tests/pr-to-ready/resolve-thread_test.sh
...
# others.
#
#   git show a05a84f^:ai/skills/pr-to-ready/scripts/resolve-thread.sh >"$tmp/old.sh"
#   SUT="$tmp/old.sh" ai/tests/run.sh ai/tests/pr-to-ready/resolve-thread_test.sh
...
SUT="${SUT:-${REPO_ROOT}/ai/skills/pr-to-ready/scripts/resolve-thread.sh}"
