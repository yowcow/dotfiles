#!/usr/bin/env bash
# Table test for ai/skills/pr-to-ready/scripts/list-unresolved-threads.sh: each
...
# exited 1 where the contract now says 2) — see ai/tests/README.md:
#   tmp="$(mktemp -d)"
#   git show a6d7ed9^:ai/skills/pr-to-ready/scripts/list-unresolved-threads.sh >"$tmp/old.sh"
#   SUT="$tmp/old.sh" ai/tests/run.sh ai/tests/pr-to-ready/list-unresolved-threads_test.sh
set -euo pipefail
...
SUT="${SUT:-${REPO_ROOT}/ai/skills/pr-to-ready/scripts/list-unresolved-threads.sh}"
