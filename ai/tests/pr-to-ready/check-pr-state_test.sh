#!/usr/bin/env bash
# Table test for ai/skills/pr-to-ready/scripts/check-pr-state.sh: the three
...
#
# RED verification (see ai/tests/README.md) -- the origin-mismatch row must fail
...
#   tmp="$(mktemp -d)"
#   git show a548e36^:ai/skills/pr-to-ready/scripts/check-pr-state.sh >"$tmp/old.sh"
#   SUT="$tmp/old.sh" ai/tests/run.sh ai/tests/pr-to-ready/check-pr-state_test.sh
set -euo pipefail
...
SUT="${SUT:-${REPO_ROOT}/ai/skills/pr-to-ready/scripts/check-pr-state.sh}"
