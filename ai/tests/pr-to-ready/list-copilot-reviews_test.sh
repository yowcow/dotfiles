#!/usr/bin/env bash
# Table test for ai/skills/pr-to-ready/scripts/list-copilot-reviews.sh: each row
...
#
# The copy must live in ai/tests/pr-to-ready/ — it sources ../lib/harness.sh off
# its own dirname — and must be removed afterwards, since lint.sh selects by
# shebang and run.sh collects *_test.sh. See ai/tests/README.md.
#   tmp="$(mktemp -d)"
#   MUT='s/(\.author\.login \/\/ "") | ascii_downcase/(.author.login) | ascii_downcase/'
#   sed "$MUT" ai/skills/pr-to-ready/scripts/list-copilot-reviews.sh >"$tmp/mut.sh"
#   sed "$MUT" ai/tests/pr-to-ready/list-copilot-reviews_test.sh >ai/tests/pr-to-ready/mut_test.sh
#   SUT="$tmp/mut.sh" ai/tests/run.sh ai/tests/pr-to-ready/mut_test.sh
#   rm ai/tests/pr-to-ready/mut_test.sh
set -euo pipefail
...
SUT="${SUT:-${REPO_ROOT}/ai/skills/pr-to-ready/scripts/list-copilot-reviews.sh}"
