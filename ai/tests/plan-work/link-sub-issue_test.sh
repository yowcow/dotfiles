#!/usr/bin/env bash
# Table test for ai/skills/plan-work/scripts/link-sub-issue.sh: the two guards
...
#
# RED verification (see ai/tests/README.md) — this script has no usable pre-fix
# version in history, so detection power is shown by removing one named guard at
# a time from a copy:
#   tmp="$(mktemp -d)"
#   cp ai/skills/plan-work/scripts/link-sub-issue.sh "$tmp/mut.sh"
#   # guard 1: replace CHILD_ID=$(gh api ...) with CHILD_ID="$CHILD"
#   # guard 2: delete the read-back and the grep -qx block
#   SUT="$tmp/mut.sh" ai/tests/run.sh ai/tests/plan-work/link-sub-issue_test.sh
set -euo pipefail
...
SUT="${SUT:-${REPO_ROOT}/ai/skills/plan-work/scripts/link-sub-issue.sh}"
