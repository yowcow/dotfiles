#!/usr/bin/env bash
# Table test for ai/skills/plan-work/scripts/list-sub-issues.sh: the guards its
...
#   tmp="$(mktemp -d)"
#   cp ai/skills/plan-work/scripts/list-sub-issues.sh "$tmp/mut.sh"
#   # guard 1: drop --paginate     guard 2: send per_page through -F
#   SUT="$tmp/mut.sh" ai/tests/run.sh ai/tests/plan-work/list-sub-issues_test.sh
set -euo pipefail
...
SUT="${SUT:-${REPO_ROOT}/ai/skills/plan-work/scripts/list-sub-issues.sh}"
