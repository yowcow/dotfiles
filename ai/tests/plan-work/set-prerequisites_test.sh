#!/usr/bin/env bash
# Table test for ai/skills/plan-work/scripts/set-prerequisites.sh: the guard its
...
# really runs.
#
# RED verification (see ai/tests/README.md) — no usable pre-fix version exists,
# so detection power is shown by removing one named guard at a time:
#   tmp="$(mktemp -d)"
#   cp ai/skills/plan-work/scripts/set-prerequisites.sh "$tmp/mut.sh"
...
#   SUT="$tmp/mut.sh" ai/tests/run.sh ai/tests/plan-work/set-prerequisites_test.sh
set -euo pipefail
...
SUT="${SUT:-${REPO_ROOT}/ai/skills/plan-work/scripts/set-prerequisites.sh}"
