#!/usr/bin/env bash
# Tests ai/tests/scripts-have-tests.sh — the coverage gate that fails the suite
# when a script lands under ai/skills/*/scripts/ with no test file and no
# allowlist line.
#
...
# the gate reaches `make -C ai test` without a Makefile or workflow change —
...
# $HARNESS_TMP: <root>/ai/skills/<skill>/scripts/... and <root>/ai/tests/...
...
SUT="${SUT:-${REPO_ROOT}/ai/tests/scripts-have-tests.sh}"
...
# natural spelling once the RED copy sits beside the gate, `SUT=ai/tests/mutA.sh`
...
# because the gate derives ai/skills and ai/tests from the root it is handed; a
# flatter fixture would test a path derivation nobody uses.
tree_new() {
  local root
  root="$(mktemp -d "${HARNESS_TMP}/tree.XXXXXX")"
  mkdir -p "${root}/ai/skills" "${root}/ai/tests"
  printf '%s\n' "$root"
}

# mk_script <root> <skill> <name>
mk_script() {
  mkdir -p "$1/ai/skills/$2/scripts"
  printf '#!/usr/bin/env bash\nexit 0\n' >"$1/ai/skills/$2/scripts/$3"
}

# mk_test <root> <skill> <name>   — <name> is the test file's own basename
mk_test() {
  mkdir -p "$1/ai/tests/$2"
  printf '#!/usr/bin/env bash\nexit 0\n' >"$1/ai/tests/$2/$3"
}
...
# — the failure ai/tests/lint.sh's header records.
total=$((total + 1))
...
mk_test "$root" alpha 'a_test.sh'
mk_allowlist "$root" <<'AL'
ai/skills/alpha/scripts/gone.sh
AL
run_sut bash "$SUT" "$root"
...
if ! check_bytes 'unnecessary entry: stdout' \
  'scripts-have-tests: allowlist entry no longer needed (the script has a test): ai/skills/alpha/scripts/a.sh\nscripts-have-tests: 1 script(s), 1 with tests, 0 exempted by the allowlist\n'; then fails_here=1; fi
if [ "$fails_here" -ne 0 ]; then failed=$((failed + 1)); fi
...
# `# ai/skills/...` read as a literal entry does not match the bare path either
...
# Measured in ai/tests/lint.sh and ai/tests/run.sh: a chmod-000 subdirectory
# makes `find` die partway, and a loop fed by process substitution reports the
# files it did collect as a complete, passing run.
root="$(tree_new)"
mk_script "$root" alpha 'a.sh'
mk_test "$root" alpha 'a_test.sh'
mkdir -p "${root}/ai/skills/beta/scripts/hidden"
printf '#!/usr/bin/env bash\nexit 0\n' >"${root}/ai/skills/beta/scripts/hidden/b.sh"
chmod 000 "${root}/ai/skills/beta/scripts/hidden"
if [ -r "${root}/ai/skills/beta/scripts/hidden" ]; then
...
ai/skills/alpha/scripts/a.sh
AL
chmod 000 "${root}/ai/tests/scripts-have-tests.allowlist"
if [ -r "${root}/ai/tests/scripts-have-tests.allowlist" ]; then
...
fails_here=0
root="$(tree_new)"
mkdir -p "${root}/ai/skills/alpha/scripts/lib"
...
# ai/skills/<skill>/references/*.md and SKILL.md are not scripts, and demanding
...
mk_script "$root" alpha 'a.sh'
mk_test "$root" alpha 'a_test.sh'
mkdir -p "${root}/ai/skills/alpha/references/scripts"
printf 'prose\n' >"${root}/ai/skills/alpha/references/base.md"
printf 'prose\n' >"${root}/ai/skills/alpha/SKILL.md"
printf '#!/usr/bin/env bash\nexit 0\n' >"${root}/ai/skills/alpha/references/scripts/deep.sh"
run_sut bash "$SUT" "$root"
