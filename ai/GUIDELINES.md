...
- Before writing code, declare which execution method you chose and why — this choice is explicit, never an implicit drift into doing it yourself:
...
- Delegation pays off only when each task is large enough to amortize the handoff (context packaging, the worker re-reading files, and review); inline trivially small independent changes rather than paying that overhead per task.
