...
# giving up. Missing the remote-only case would cut a fresh branch from the
# default instead, leaving already-pushed work stranded as a separate history
# under the same name (force-push is banned, so there is no recovering it).
...
if git fetch origin "${BRANCH}" >&2 && git show-ref --verify --quiet "refs/remotes/origin/${BRANCH}"; then
