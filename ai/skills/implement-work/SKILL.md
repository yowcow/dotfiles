...
4. **Commit** whatever steps 2 and 3 changed, in the same round that changed it. A round that changed nothing commits nothing. This sits before the exits below so that **every** exit leaves a clean tree, because both of them hand over the branch rather than the worktree: **Hand off** calls `superpowers:finishing-a-development-branch`, whose push and merge options act on *commits*, so an uncommitted edit reaches neither — and it goes down with the worktree whenever that is removed. **Escalation** hands `plan-work` a branch name, and re-approval is judged against what that branch contains, so an edit left uncommitted is simply absent from what the next flow reads.
