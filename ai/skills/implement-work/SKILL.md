...
4. **Commit** whatever steps 2 and 3 changed, in the same round that changed it. A round that changed nothing commits nothing. This sits before the exits below so that **every** exit leaves a clean tree: **Hand off** calls `superpowers:finishing-a-development-branch`, which runs the project's checks and presents integration options against the working tree it finds — uncommitted edits there are pushed by no option and are lost with the worktree — and **Escalation** hands `plan-work` a branch whose state it has to describe, which an uncommitted edit makes undescribable.
