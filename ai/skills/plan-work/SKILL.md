---
name: plan-work
description: Use to turn an issue or a planning request into an agreed design and a numbered TODO list at PR granularity, before any code is touched. Researches the issue and the relevant code, reaches design agreement with `superpowers:brainstorming`, drafts the TODO list itself, runs the `review-plan` loop to convergence, publishes once as an issue comment (or in chat), and creates one sub-issue per TODO item. Triggers on "plan this", "plan issue #<n>", "turn this into a plan", "break this into PRs", "brainstorm and plan this".
---
