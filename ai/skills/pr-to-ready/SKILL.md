...
3. Copilot's latest round produced zero new actionable comments, and `<skill-dir>/scripts/list-unresolved-threads.sh <owner> <repo> <pr-number>` **exits 0 with no output** — a comment listing carries no resolution state, so only that script answers this;
