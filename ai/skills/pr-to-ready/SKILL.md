...
3. **LGTM-equivalent twice in a row** — even if each round keeps surfacing *fresh optional nits*, once you've gotten two consecutive rounds with no must-fix feedback, stop; endless optional-nit chasing is not required for ready. This is a stricter condition on top of *clean*, which the guidelines' **Loop convergence** allows and does not override: external reviewers vary from round to round, so one quiet round is weaker evidence here than it is elsewhere.
