...
Then watch CI with `<skill-dir>/scripts/watch-checks.sh <owner> <repo> <sha>`, which answers one line per check — name, status, conclusion — once none is queued or running, and fails rather than answering when the SHA is not on the remote. Whether those conclusions pass is read here, never by the script. If every check passes, go to Step 2.
